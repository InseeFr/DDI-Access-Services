...
public class RessourcePackage {
    ...
    public List<Map<String, String>> mapCodesToLabels() {
        ...
        for (int i = 0; i < cls.getCodeLists().size(); i++) {
            CodeListScheme.CodeList currentCodeList = cls.getCodeLists().get(i);
            List<CodeListScheme.CodeList.Code> codes = currentCodeList.getCodes();

            // Itérer sur chaque code dans la CodeList actuelle
            for (int j = 0; j < codes.size(); j++) {
                Map<String, String> mapping = new HashMap<>();
                CodeListScheme.CodeList.Code currentCode = codes.get(j);
